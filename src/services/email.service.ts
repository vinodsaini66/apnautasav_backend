...
export class EmailService {
  ...
  /**
   * Password reset link (AuthService.forgotPassword). Same pattern as
   * sendVerificationEmail — `resetLink` is a FRONTEND_URL page that reads
   * the `token` query param and calls POST /auth/reset-password itself.
   */
  static async sendPasswordResetEmail(to: string, fullName: string, resetLink: string): Promise<boolean> {
    return this.sendMail(
      to,
...
function passwordResetEmailTemplate(fullName: string, resetLink: string): string {
  return `
<!doctype html>
<html>
  <body style="margin:0;padding:0;background-color:#fff6ef;font-family:Georgia,'Times New Roman',serif;">
      ...
      <tr>
        <td align="center">
          <table role="presentation" width="100%" style="max-width:520px;background-color:#ffffff;border-radius:20px;overflow:hidden;box-shadow:0 4px 24px rgba(200,30,74,0.08);">
            ...
            <tr>
              <td style="padding:32px 32px 8px;">
                <p style="margin:0 0 4px;font-size:12px;font-weight:bold;letter-spacing:0.08em;color:#c81e4a;text-transform:uppercase;">Password reset</p>
                <h1 style="margin:0 0 16px;font-size:22px;line-height:1.3;color:#1e1b1f;">Reset your password, ${escapeHtml(fullName)}</h1>
                <p style="margin:0 0 24px;font-size:15px;line-height:1.6;color:#4b4b52;">
                  We received a request to reset the password for your ApnaUtsav account. Click below to set a new
                  one — your wedding, guests and budget are untouched either way.
                </p>
                <table role="presentation" width="100%" style="margin:0 0 24px;">
                  <tr>
                    <td align="center">
                      <a href="${resetLink}" style="display:inline-block;padding:14px 32px;border-radius:999px;background:linear-gradient(135deg,#7a1330,#c81e4a,#fb4d61);color:#ffffff;font-family:Arial,Helvetica,sans-serif;font-size:15px;font-weight:bold;text-decoration:none;">
                        Set a new password
                      </a>
                    </td>
                  </tr>
                ...
                <p style="margin:0 0 24px;font-size:13px;line-height:1.6;word-break:break-all;">
                  <a href="${resetLink}" style="color:#c81e4a;">${resetLink}</a>
                </p>
                <p style="margin:0 0 8px;font-size:13px;line-height:1.6;color:#9a9aa0;">
                  This link works once and expires in an hour. If you didn't request this, you can safely ignore
